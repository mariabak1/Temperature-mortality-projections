### Code to compute temperature-related mortality for ages 0-49 ####
### while considering changes in population size, age group size, and mortality rates in mid-century ###
### Age-group specific weights are read in during MS236_Read_in_data_age049.R load ###
...
# load the data
source("MS236_Read_in_data_age049.R")
...
ANSIM = list() # store simulation results
DEATHDENOM=list()
DEATHDENOMH=list()
...
for (c in 1:length(unique(cities$nsalid1))){
  red=PREDICTED.0TO49$PredictedMV1[[c]] 
  #cen=PREDICTED.ALLAGES$MMT$MMT1[[c]]
  cen=PREDICTED.0TO49$MMT$MMT1[[c]]
  coef <- coef(red)
  vcov <- vcov(red)
  ...
  deathdoy = DEATHDOY.AGE2[[c]]$`deathdoy.0-49`
  ...
  #sd(apply(ansim[,,"abs",,"RCP2.6",1],1:2,mean))
  ...
  # update: since implementing Sarav's temperature idea, this is no longer the case, but I left this as is for flexibility
  
  afabs[histperiod[1],,,] <- anabs[histperiod[1],,,]/deathperiod_hist*100
  afabs["2045-2054",,,] <- anabs["2045-2054",,,]/deathperiod*100
  
  #sd(apply(ansim[,,"abs",,"RCP2.6",1],1:2,mean))/deathperiod_hist*100
...
write.csv(ansim, "age_specific/ansim049mmt.csv", row.names = F)
write.csv(afs, "age_specific/afabs049mmt.csv", row.names = F)
write.csv(afrels, "age_specific/afrels049mmt.csv", row.names = F)
write.csv(ans, "age_specific/anabs049mmt.csv", row.names = F)
write.csv(anrels, "age_specific/anrels049mmt.csv", row.names = F)

write.csv(df, "age_specific/df049mmt.csv", row.names = F)
write.csv(dh, "age_specific/dh049mmt.csv", row.names = F)
