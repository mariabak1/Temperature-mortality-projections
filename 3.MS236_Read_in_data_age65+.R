...
for (c in  1:length(unique(deathspercity$nsalid1))){
  #c=1
    w=brw[[c]]
    w=w$brw[w$a == '65+']
    DEATHPROJ[[c]] = rep(DEATHDOY.AGE2[[c]]$`deathdoy.65+` * w, length = length(rcp2p6[[c]]$date)) 
}
...
for (c in  1:length(unique(deathspercity$nsalid1))){
  #c=1
  DEATHPROJ_HIST[[c]] = rep(DEATHDOY.AGE2[[c]]$`deathdoy.65+`, length = 365*length(seq(dailytempcity[[c]]$start.year[1], dailytempcity[[c]]$end.year[1]))) 
}
