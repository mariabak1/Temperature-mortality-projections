...
load("temp_mortality_projections/small_all cause p10p75p90 lag21.Rdata")
...
# this is the part to which weights are applied (from the brw); 
DEATHPROJ = list()
for (c in  1:length(unique(deathspercity$nsalid1))){
  #c=1
  w=brw[[c]]
  w=w$brw[w$a == '0-49']
  DEATHPROJ[[c]] = rep(DEATHDOY.AGE2[[c]]$`deathdoy.0-49` * w, length = length(rcp2p6[[c]]$date)) 
}
...
for (c in  1:length(unique(deathspercity$nsalid1))){
  #c=1
  DEATHPROJ_HIST[[c]] = rep(DEATHDOY.AGE2[[c]]$`deathdoy.0-49`, length = 365*length(seq(dailytempcity[[c]]$start.year[1], dailytempcity[[c]]$end.year[1]))) 
}
