### Code to compute temperature-related mortality for ages 50-64 ####
### while considering changes in population size, age group size, and mortality rates in mid-century ###
### Age-group specific weights are read in during MS236_Read_in_data_age5064.R load ###
...
# load the data
source("MS236_Read_in_data_age5064.R")
...
for (c in 1:length(unique(cities$nsalid1))){
  ...
  red=PREDICTED.50TO64$PredictedMV1[[c]] 
 # cen=PREDICTED.ALLAGES$MMT$MMT1[[c]]
  cen=PREDICTED.50TO64$MMT$MMT1[[c]]
  ...
  deathdoy = DEATHDOY.AGE2[[c]]$`deathdoy.50-64`
  ...
  # update: since implementing Sarav's temperature idea, this is no longer the case, but I am computing them separately for now
...
write.csv(ansim, "age_specific/ansim5064.csv", row.names = F)
write.csv(afs, "age_specific/afabs5064.csv", row.names = F)
write.csv(afrels, "age_specific/afrels5064.csv", row.names = F)
write.csv(ans, "age_specific/anabs5064.csv", row.names = F)
write.csv(anrels, "age_specific/anrels5064.csv", row.names = F)

write.csv(df, "age_specific/df5064.csv", row.names = F)
write.csv(dh, "age_specific/dh5064.csv", row.names = F)
